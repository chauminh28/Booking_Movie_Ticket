...
function Checkout() {
    ...
    const handlePayment = async () => {
        try {
            ...
            const res = await axiosClient.post("/bookings", form);
            alert(`Bạn đã chọn thanh toán bằng ${paymentMethod.toUpperCase()}`);
        } catch (error) {
    ...
    return (
        <>
            ...
            <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-6 mb-10">
                ...
                <div className="mb-4">
                    <h3 className="text-lg font-semibold mb-2">Suất chiếu: {schedule.scheduleDate} lúc {showTimeId.time}</h3>
                </div>
                ...
                <div className="mb-4">
                    ...
                    {Object.keys(serviceQuantities).length === 0 ? (
                        ...
                        <ul className="list-disc list-inside">
                            {Object.entries(serviceQuantities).map(([productId, quantity]) => {
                                const product = products.find(p => p.id === parseInt(productId));
                                if (!product) return null;

                                return (
                                    <li key={productId}>
                                        {product.serviceName} - {quantity} cái ({(product.price * quantity).toLocaleString()} VND)
                                    </li>
                                );
                            })}
                        </ul>
                ...
                <div className="mb-4">
                    ...
                    <p className="text-xl font-bold text-green-600">{totalPrice.toLocaleString()} VND</p>
                </div>
